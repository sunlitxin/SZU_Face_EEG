...
## Usage：此文件是将edf读取为pkl文件，可以通过调节参数stim_length 和 slice_set_nums分别选择读取edf的 time-length以及对 time-length进行几等分的切分
...
if __name__ == "__main__":
    ...
    for base_name, files in edf_files.items():
        ...
        # 并行处理EDF文件
        parallel_process_files([edf_file_path], label_file_path, stim_length, output_pkl_dir, slice_set_nums, n_jobs=4)
